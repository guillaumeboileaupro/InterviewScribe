...
Small, Medium et Large v3 Turbo (Q5_0/Q5_1) restent fournis et
selectionnables explicitement (12 septembre 2026 : Tiny et Medium ajoutes
pour combler les deux extremes manquants - le plus rapide et un palier
intermediaire avant Large v3 Turbo). Ce choix de defaut evite qu'une courte
transcription paraisse bloquee sur une machine CPU: lors du diagnostic du
10 septembre 2026, Base a traite 26,9 s d'audio en 13,0 s, tandis que
Large v3 Turbo depassait 90 s sur le meme poste (voir aussi "Acceleration
GPU" plus haut, qui reduit cet ecart sur une machine avec GPU compatible
Vulkan). Les revisions, tailles et empreintes SHA-256 sont epinglees dans
les manifestes de `src-tauri/resources/models/`, chacune verifiee
reellement par telechargement et `sha256sum` avant d'etre epinglee, pas
seulement copiee depuis les metadonnees LFS de la source. La licence MIT
de Whisper est livree avec eux.

Le Large v3 complet (non-turbo, ~1,08 Go) a ete ajoute puis retire le
12 septembre 2026 dans la meme session : l'embarquer en plus des cinq
autres a fait depasser aux trois paquets (.deb, installateur NSIS, APK) la
limite reelle de GitHub Releases de 2 Gio par fichier, provoquant l'echec
complet d'une tentative de publication v0.3.0
("size must be less than 2147483648" sur les trois plateformes). Non
reintroduit tant qu'aucune strategie de distribution ne permet de
depasser cette limite (paquets separes, telechargement optionnel post-
installation, etc. - aucune de ces pistes n'est engagee).
...
Limites non couvertes: les tests de mise a niveau `.deb` et NSIS ainsi que la preservation des donnees a la desinstallation sont automatises mais attendent leur execution native sur un tag; l'aspect visuel de l'installateur (ecrans, raccourcis) n'est pas verifie par une installation silencieuse; aucune signature de code desktop (hors perimetre, aucun secret dans le depot).

### Android (Phase 6, 07/09/2026) - signee et verifiee en CI, jamais sur ce poste

Le job `android` de `release.yml` a d'abord echoue trois fois de suite pour trois raisons reelles, distinctes, trouvees uniquement en executant le pipeline (jamais supposees):
1. `ort-sys` ne compile pas du tout pour Android (voir "Diarisation" plus haut) - corrige en excluant `pyannote-rs`/`ort` de la compilation Android au niveau de `Cargo.toml`.
2. `cpal` echoue au lien avec `unable to find library -laaudio`: AAudio n'existe dans le sysroot du NDK qu'a partir de l'API 26, or le projet ciblait l'API 24. Corrige en fixant `bundle.android.minSdkVersion: 26` dans `tauri.conf.json` (Tauri propage cette valeur a la fois au `minSdk` Gradle et aux linkers Cargo - verifie en observant les variables d'environnement `CARGO_TARGET_*_LINKER` d'un vrai run apres correction).
3. `armv7-linux-androideabi` echoue differemment: `bindgen` (utilise par `whisper-rs-sys`) genere des constantes de decalage de structure incoherentes sur le NDK du runner CI (`clang diagnosed error: bits/libc-header-start.h file not found`, suivi d'erreurs `E0080` de calculs qui debordent) - signe que `bindgen` ne trouve pas le bon sysroot pour cette cible precise. Non resolu; la publication se limite a `--target aarch64` (arm64-v8a), qui couvre la quasi-totalite des appareils Android reels - `armv7` reste un axe d'amelioration futur, pas un blocage.

**Signature**: un keystore de test est genere localement (`keytool`, jamais commis) et stocke uniquement en secrets GitHub Actions (`ANDROID_KEYSTORE_BASE64` encode en base64, plus mot de passe et alias). `scripts/android-sign.sh` decode le keystore et ecrit `gen/android/app/keystore.properties`, puis `scripts/android-sign-patch.py` insere un bloc `signingConfigs` dans `build.gradle.kts` - **repete a chaque build**, jamais une edition manuelle unique, car `gen/android/` est regenere integralement par `pnpm tauri android init --ci` a chaque execution (l'approche documentee par Tauri, une edition manuelle du fichier genere, ne survivrait pas). Deux bugs reels trouves en executant ce script pour de vrai: un import Kotlin manquant (`java.util.Properties`/`java.io.FileInputStream` qualifies ne resolvent pas dans ce contexte de lambda - corrige en import non qualifie, cohere avec le style du fichier genere par Tauri) et un chemin de fichier incorrect (`keystore.properties` doit vivre a cote de `build.gradle.kts` dans `app/`, pas a la racine de `gen/android/`, car `file(...)` en Kotlin DSL resout relativement au module). La signature finale est verifiee par `apksigner verify --print-certs` sur l'APK reellement produite, pas supposee reussie parce que le build est passe.

Limites non couvertes, honnetes: un job emulateur arm64-v8a verifie desormais ABI, installation, lancement sans crash et desinstallation, mais attend encore sa premiere execution native. L'import `content://`, la transcription hors ligne, la capture microphone, le cycle de vie, l'interruption, le verrouillage, la batterie et la temperature restent non valides sur appareil physique. Pas de test de mise a niveau Android ni de modele quantifie specifique au telephone (le meme Whisper Turbo Q5_0 que le bureau est utilise).
