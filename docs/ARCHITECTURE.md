...
Limites non couvertes: pas de test de mise a niveau (installer une nouvelle version par-dessus une ancienne); l'aspect visuel de l'installateur (ecrans, raccourcis) n'est pas verifie par une installation silencieuse; aucune signature de code desktop (hors perimetre, aucun secret dans le depot).
...
Limites non couvertes, honnetes: aucun test sur appareil ou emulateur Android reel (import `content://` en conditions reelles, capture microphone, cycle de vie, interruption, verrouillage d'ecran, batterie, temperature) - aucun materiel disponible dans cet environnement de developpement. Pas de test de mise a niveau. Pas de modele quantifie specifique au telephone (le meme Whisper Turbo Q5_0 que le bureau est utilise).
